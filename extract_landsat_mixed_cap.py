...
def pc_query(sample):
    ...
    search = catalog.search(collections=["landsat-c2-l2"], bbox=bb, datetime=sampling_period, query={"platform": {"in": ["landsat-7", "landsat-8"]}, "eo:cloud_cover": {"lt": 60}})
    ...
    items = sorted(items, key=lambda x: x.properties["datetime"])
    items = items[-25:]
